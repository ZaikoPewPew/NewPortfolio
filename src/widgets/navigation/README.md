...
- Auto-hide (асимметричный, как тулбары article-сайтов): скрытие после ~72px накопленного скролла вниз, показ мгновенно при скролле вверх; всегда видна в зоне < 120px от верха; у низа не показывается сама — только по движению вверх
- Синхронно с хедером кейса (`caseChrome.client.ts`) — один контроллер; поведение не зависит от открытости ThemeWidget
