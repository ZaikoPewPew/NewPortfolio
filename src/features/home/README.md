...
3. Класс `is-case-active` (на `html` / `[data-home-page]`) увеличивает размер currently-block; карточка получает `.is-active`
...
| Me flip | `:has([data-me-flipping])` поднимает `grid__widgets` над header glass (`z-header + 1`) |

На странице кейса (`data-case-page`, desktop): без choreography — мгновенный переход; виджеты скрыты (`visibility` + absolute), cases на ширину `header__row`, cover `--layout-case-cover-height` с отступом `--layout-case-cover-inset-top` (16px от верха viewport), текст `--layout-case-reading-width` (900px) по центру. Mobile без изменений.
...
6. Me flip: кнопка остаётся кликабельной; enter/leave и bubble глушатся на `[data-me-flipping]`, после анимации — `resumeContactButtonHover()` (см. [`widgets/me/README.md`](../widgets/me/README.md))
