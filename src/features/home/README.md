...
- `case-hover/` — фокус карточки: blur/wash, подъём карточки, currently-block — см. [`case-hover/README.md`](case-hover/README.md)
...
1. `CaseCard` ставит `data-case-card` и `data-hover-*` атрибуты (включая `data-hover-video`)
...
3. Градиент → CSS var `--page-gradient` на `:root`; blur/wash — класс `is-case-active`
4. Активная карточка **поднимается в фокус** над blur, рядом летит currently-block с видео
5. Теги на карточке появляются при `.is-active` / `:focus-visible`; `CasePreviewPanel` показывает preview

Детали (порталы, elevate, логика выхода, scale блока) — [`case-hover/README.md`](case-hover/README.md).
