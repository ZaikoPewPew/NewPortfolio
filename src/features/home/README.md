...
- `background/` — PageBackground (фон + hover-градиент/noise); монтируется в `BaseLayout`
- `case-hover/` — hover карточки: currently-block + градиент фона — см. [`case-hover/README.md`](case-hover/README.md)
...
1. `CaseCard` ставит `data-case-card`, `data-hover-from/to/angle` и `data-hover-video`
...
3. Класс `is-case-active` (на `html` / `[data-home-page]`) включает градиент+noise фона, увеличивает currently-block и курсор «open»; карточка получает `.is-active`
4. Рядом с курсором летит currently-block с видео **с начала** при каждом входе на плитку; оверлеев blur/wash/preview нет
